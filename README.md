...
| [`planning-and-decomposition/`](planning-and-decomposition/) | Real plan-and-execute repros: a replanning trigger (honest negative — Sonnet 5 adapted to a broken plan assumption either way) and a granularity tradeoff that produced a real, reproducible failure — an over-granular plan burned its entire token budget on plan text and never executed a tool — caught by a minimal verifier-in-the-loop check | ✅ |
...
| [`observability-ecosystem/`](observability-ecosystem/) | A real, live, zero-cost test of two observability platform architectures — Langfuse's real OTLP endpoint rejected an unauthenticated real OTel GenAI trace with a real 401, and Helicone's real gateway genuinely forwarded a live request to OpenAI's real API and passed its real error back through | ✅ |
...
| [`computer-use-browser-agents/`](computer-use-browser-agents/) | A real, safe repro of the CometJacking-class "summarize this webpage" injection mechanism — a fictional page with a hidden data-harvesting pretext, tested raw vs. tagged; Sonnet 5 resisted in both conditions, a real honest negative reached after removing an unrealistic self-announcing tell from the injection | ✅ |
