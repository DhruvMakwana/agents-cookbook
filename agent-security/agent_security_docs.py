"""
Same logic as agent_security.py, split into self-contained blocks with no
...
def _run_agent_loop(question, tools, impls, client, model, max_steps=6) -> dict:
    ...
    for _ in range(max_steps):
        ...
        if response.stop_reason != "tool_use":
            return {"answer": _text(response, client, model), "trace": trace}
        tool_results = []
...
# an instruction to an assistant at all. Real attacks don't announce
# themselves the way an earlier, more obvious draft of this email did.
_INBOUND_EMAIL = (
...
def trifecta_demo(client, model) -> dict:
    global _SENT_LOG
    _SENT_LOG = []
    ...
    return {
        "vulnerable": {"answer": vulnerable["answer"], "trace": vulnerable["trace"], "exfiltrated": vulnerable_exfiltrated},
        "fixed": {"answer": fixed["answer"], "trace": fixed["trace"], "exfiltrated": fixed_exfiltrated},
    }
# --8<-- [end:trifecta]


# --8<-- [start:tool_poisoning]
import re
...
def tool_poisoning_demo(client, model) -> dict:
    impls = {"get_weather": get_weather, "lookup_customer_record": lookup_customer_record}
    question = "What's the weather in Paris?"

    vulnerable = _run_agent_loop(question, [_WEATHER_TOOL_POISONED, _CUSTOMER_LOOKUP_TOOL], impls, client, model)
    ...
    fixed = _run_agent_loop(question, [sanitized_tool, _CUSTOMER_LOOKUP_TOOL], impls, client, model)
